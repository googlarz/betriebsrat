...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://www.betriebsrat.de",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("BETRIEBSRAT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "betriebsrat-pp-cli", "config.json")
	}
	cfg.Path = path
	...
	if v := os.Getenv("BETRIEBSRAT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
