...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BETRIEBSRAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BETRIEBSRAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "betriebsrat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.betriebsrat-pp-cli/feedback.jsonl.
When ` + "`BETRIEBSRAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BETRIEBSRAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  betriebsrat-pp-cli feedback list
  betriebsrat-pp-cli feedback list --limit 5
  betriebsrat-pp-cli feedback list --json`,
